...
import com.cjw.demo.doc.address.entity.DataConstant;
import com.cjw.demo.doc.util.StringUtils;
...
public class Address {
	...
	private static final List<String> branchSchoolWordsList = Arrays.asList(DataConstant.BRANCH_SCHOOL_WORDS.split(","));
	...
	public List<List<Address>> breakTree() {
	...
	public Info info() {
        ...
        fixDetailAddress(res);
        return res;
	}

    /**
     * 修复由于地址里带(校区,分校,等等)产生的bug
     * @param info
     * @return
     */
	private void fixDetailAddress(Info info) {
        final String provinceAddress = info.getProvinceAddress();
        if (StringUtils.isNotBlank(provinceAddress)) {
            String originalAddress = info.getOriginalAddress();
            for (String branchSchoolWord : branchSchoolWordsList) {
                if (originalAddress.contains("大学") && originalAddress.contains(branchSchoolWord)) {
                    String detailAddress = originalAddress;
                    final String cityAddress = info.getCityAddress();
                    final String areaAddress = info.getAreaAddress();
                    if (!dCity.contains(provinceAddress)) {
                        detailAddress = detailAddress.replace(provinceAddress,"");
                    }
                    if (StringUtils.isNotBlank(cityAddress)) {
                        detailAddress = detailAddress.replace(cityAddress,"");
                    }
                    if (StringUtils.isNotBlank(areaAddress)) {
                        detailAddress = detailAddress.replace(areaAddress,"");
                    }
